...
module Orchid
  ...
  def self.sequence(seq)
    ...
    # Otherwise, dump the sequence to a FASTA file and use the FASTA Orchid script
      ...
      begin
        ...
        raise OrchidError, "Error running orchid script!"
      ensure
  ...
  def self.fasta(filename)
    Perl.run(ORCHID_FASTA_SCRIPT, ['-f', File.expand_path(filename)]).split("\n")
  end
end

class OrchidError < PerlError
end
