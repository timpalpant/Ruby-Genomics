...
shared_examples "a scripts collection" do |dir|
  ...
  Dir.glob(dir + '/*.xml').each do |f|
    ...
    describe "#{config.command.script_name}: #{config.long_name}", :if => (File.exist?(full_path) and config.tests.length > 0) do
      before do
        @runner = GalaxyTestRunner.new(config)
      end
      
      it "generating outputs" do
        @runner.generate_test_outputs(File.expand_path(File.dirname(__FILE__) + '/test-data'))
      end
    end
  end
end
